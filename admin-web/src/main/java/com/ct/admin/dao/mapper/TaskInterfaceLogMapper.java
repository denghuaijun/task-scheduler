...
public interface TaskInterfaceLogMapper {
    ...
    @Insert({
        "insert into task_interface_log (task_request_path, task_request_param, ",
        "task_reponse_param, task_request_time, ",
        "task_reponse_time, task_session_id, ",
        "task_client_ip, task_request_method, ",
        "task_request_type, task_reponse_code, ",
        "create_time, remark)",
        "values (#{taskRequestPath,jdbcType=VARCHAR}, #{taskRequestParam,jdbcType=VARCHAR}, ",
        "#{taskReponseParam,jdbcType=VARCHAR}, #{taskRequestTime,jdbcType=TIMESTAMP}, ",
    ...
    int insert(TaskInterfaceLog record);

    @InsertProvider(type=TaskInterfaceLogSqlProvider.class, method="insertSelective")
    @SelectKey(statement="SELECT LAST_INSERT_ID()", keyProperty="id", before=false, resultType=Long.class)
    int insertSelective(TaskInterfaceLog record);
    ...
    @Results({
        ...
        @Result(column="task_request_param", property="taskRequestParam", jdbcType=JdbcType.VARCHAR),
        @Result(column="task_reponse_param", property="taskReponseParam", jdbcType=JdbcType.VARCHAR),
    ...
    @Select({
        "select",
        "id, task_request_path, task_request_param, task_reponse_param, task_request_time, ",
        "task_reponse_time, task_session_id, task_client_ip, task_request_method, task_request_type, ",
        "task_reponse_code, create_time, remark",
    ...
    TaskInterfaceLog selectByPrimaryKey(Long id);

    @UpdateProvider(type=TaskInterfaceLogSqlProvider.class, method="updateByExampleSelective")
    int updateByExampleSelective(@Param("record") TaskInterfaceLog record, @Param("example") TaskInterfaceLogExample example);
    ...
    int updateByPrimaryKeySelective(TaskInterfaceLog record);

    @Update({
        "update task_interface_log",
        "set task_request_path = #{taskRequestPath,jdbcType=VARCHAR},",
          "task_request_param = #{taskRequestParam,jdbcType=VARCHAR},",
          "task_reponse_param = #{taskReponseParam,jdbcType=VARCHAR},",
